...
path_to_folder = paste(path_to_output, 'fPCA_ac', sep = '/')
...
for (i_asset in 1:N_assets) {# Get assets values in matrix form
  ...
  Y_tr_ = Y_[idx_tr_, 1:N_fc_event]
  Y_ts_ = Y_[idx_ts_, 1:N_fc_event]
  X_tr_ = Y_[idx_tr_, ]
  X_ts_ = Y_[idx_ts_, ]
  dim(Y_tr_)
  dim(Y_ts_)
  ...
  t_ = seq(1, N_fc_event, length.out = N_fc_event)
  length(t_)
  
  # Perform fPCA
  train = MakeFPCAInputs(IDs  = rep(1:N_tr_samples, each = N_fc_event), 
