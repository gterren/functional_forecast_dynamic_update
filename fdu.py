...
class functional_dynamic_update:
    ...
    def _neighborhood_filtering(self, w_, d_h_, d_p_, gamma, xi, kappa):
        ...
        if (idx_temporal_.shape[0] < kappa) or (d_h_ is None):  
    ...
    def fit(self, F_, E_, dt_, 
        ...
        if self._distances['temporal'] == 'seasonal':
            self.d_p_ = self._seasonal_dist(self.t_, self.t)
        elif self._distances['temporal'] == 'seasonal_equinox':
            self.d_p_ = self._seasonal_equinox_dist(self.t_, self.t)
        else:
            self.d_p_ = None
        ...
        if self._distances['spatial'] == 'euclidean':
            self.d_h_ = self._weighted_euclidian_dist(self.X_, self.x_)
        elif self._distances['spatial'] == 'haversine':
            self.d_h_ = self._haversine_dist(self.X_, self.x_)
        else:
            self.d_h_ = None

    def predict(self, f_, e_,
                forget_rate_f = 1.,
                forget_rate_e = .5,
                lookup_rate = .05,
                length_scale_f = .1,
        ...
        print(psi_minus_.shape, psi_plus_.shape, self.psi_.shape)
        # Mask intervals
