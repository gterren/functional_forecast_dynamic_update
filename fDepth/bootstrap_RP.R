...
print('Running bootstrap_RP.R with bootstrap confidence bands ...')
...
n_bootstrap_interations = 10 # More iterations for stable bands
...
for (i in 1:n_bands) {
  ...
  for (j in 1:n_bootstrap_interations) {
    ...
    depth_scores_   = depth.RP(fdata(bootstrap_curves_))$dep
...
depth_scores_ = depth.RP(fdata(curves_))$dep
