...
from utils import (_empirical_coverage_score,
...
time = int(sys.argv[3])
dist = sys.argv[4]
print(i_job, resource, method, time, dist)
...
for day in range(363):
    ...
    try:
        ...
        for fraction in fractions_:
            ...
            f_focal_, _upper, _lower = _fdu.focal_envelop_confidence_bands(alpha_, k_)
            
            for alpha in alpha_:

                FCS = _empirical_coverage_score(f_hat_, _lower[f'{alpha}'][1:], _upper[f'{alpha}'][1:])
                FIS = _empirical_interval_score(f_hat_, _lower[f'{alpha}'][1:], _upper[f'{alpha}'][1:], alpha).sum()

                # Save results
                FS_.append([time, asset, day, alpha, fraction, dist, M_.shape[0], J_.shape[0], FIS, FCS])
