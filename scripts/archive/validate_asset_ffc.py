...
if param == 'kappa':
    hyper_.loc['kappa', time] = [50, 75, 100, 125, 150, 175, 200, 225, 250, 275, 300, 325][i_job]

if param == 'p_fusion':
    hyper_.loc['p_fusion', time] = [0.1, 0.2, 0.3, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.8, 0.9][i_job]
...
for asset in assets_:
    for day in range(363):
        ...
        try:
            ...
            WIS = _weighted_empirical_interval_score(f_hat_, f_median_, _lower, _upper, alpha_).mean()
            PIT = _empirical_PIT(f_hat_, M_.T, seed = 1234)

            # Error metrics
            RMSE = np.sqrt(np.mean((_fdu.f_focal_ - e_[time:])**2))
            MBE = np.mean(f_hat_ - _fdu.f_focal_)
            
            # Flag sparse neighborhood
            flag  = 1*(_fdu.idx_temporal_.shape[0] < int(hyper_.loc['kappa'][time]))
            value = hyper_.loc[param, time].tolist()

            # Save results
            WIS_.append([time, asset, day, param, value, x_ts_[0], x_ts_[1], flag, WIS, RMSE, MBE, ES])
            PIT_.append(PIT)
...
KS_.append([time, region, param, value] + [_KS(PIT_[:, (i*12):(i + 1)*12].flatten()) for i in [1, 2, 3, 4, 5]])
...
if i_job == 0:
    ...
    KS_['KS'] = (KS_['KS1'] + KS_['KS2'] + KS_['KS3'] + KS_['KS4'] + KS_['KS5'])/5.

    df_ = WIS_.merge(KS_,
                     on=["resource", "method", "parameter", "value", "time"],
                     how="left")
    
    _save_validation_csv(df_, path_to_file = path_to_validation + f'/{resource}/{resource}-validation_ffc-asset-1.csv')
