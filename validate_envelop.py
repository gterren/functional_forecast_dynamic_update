...
for day in range(363):
    ...
    try:
        ...
        for fraction in fractions_:
            ...
            for alpha in alpha_:
                FCS = np.absolute(_empirical_coverage_score(f_hat_, l_[1:], u_[1:]) - (1. - alpha))
...
print(i_job, resource, dist, FCS_.shape, FIS_.shape)

FCS_ = _gather_node_data(_comm, FCS_)
FIS_ = _gather_node_data(_comm, FIS_)

if i_job == 0:
    _save_validation_csv(FCS_, path_to_file = path_to_validation + f'/{resource}-{method}-{dist}-FCS_validation_envelop_{val}.csv')
    _save_validation_csv(FIS_, path_to_file = path_to_validation + f'/{resource}-{method}-{dist}-FIS_validation_envelop_{val}.csv')